...
var UserWhere = struct {
	...
	UserId:   whereHelperint{field: "`User`.`userId`"},
	UserName: whereHelperstring{field: "`User`.`userName`"},
	UserPass: whereHelperstring{field: "`User`.`userPass`"},
}
...
var (
	userAllColumns            = []string{"userId", "userName", "userPass"}
	userColumnsWithoutDefault = []string{"userName", "userPass"}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, userId int, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from `User` where `userId`=?", sel,
	)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, userId int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `User` where `userId`=? limit 1)"
